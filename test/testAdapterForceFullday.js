...
function setupIcsFiles() {
    ...
    data += 'END:VCALENDAR\n';
    fs.writeFileSync(__dirname + '/data/force_fullday.ics', data);
}
...
describe('Test ' + adapterShortNameLog + ' adapter', function() {
    before('Test ' + adapterShortNameLog + ' adapter: Start js-controller', function (_done) {
        ...
        setup.setupController(function () {
            ...
            config.native.forceFullday = true;
    ...
    it('Test ' + adapterShortNameLog + ': data.table', function (done) {
        ...
        setTimeout(function () {
            states.getState('ical.0.data.table', function (err, state) {
                expect(err).to.be.not.ok;
                expect(state.val[0].date.substr(0, 8)).to.be.equal('&#8594; ');
                expect(state.val[0].date).to.not.have.string('. 00:00');
                ...
                expect(state.val[1].date).to.not.have.string('. 00:00', '&#8594; ');
                expect(state.val[1].date.indexOf('. 00:00')).to.be.equal(-1);
                ...
                expect(state.val[2].date.substr(0, 8)).to.be.equal('&#8594; ');
                expect(state.val[2].date).to.not.have.string('. 00:00');
                ...
                expect(state.val[3].date).to.not.have.string('. 23:58-23:59');
                ...
                expect(state.val[4].date).to.not.have.string('. 22:00-02:00+1');
                expect(state.val[4].event).to.be.equal('OverEvent');
                expect(state.val[4]._section).to.be.equal('OverEvent');
                expect(state.val[4]._allDay).to.be.true;

                expect(state.val[5].date).to.not.have.string('. 00:00');
                ...
                expect(state.val[6].date).to.not.have.string('. 18:00-20:00');
                expect(state.val[6].event).to.be.equal('InDay2');
                expect(state.val[6]._section).to.be.equal('InDay2');
                expect(state.val[6]._allDay).to.be.true;

                expect(state.val[7].date).to.not.have.string('. 10:00');
                expect(state.val[7].event).to.be.equal('Reminder');
                expect(state.val[7]._section).to.be.equal('Reminder');
                expect(state.val[7]._allDay).to.be.true;

                expect(state.val[8].date).to.not.have.string('. 19:30-20:30');
                expect(state.val[8].event).to.be.equal('TestUserEvent1');
                expect(state.val[8]._section).to.be.equal('TestUserEvent1');
                expect(state.val[8]._allDay).to.be.true;
